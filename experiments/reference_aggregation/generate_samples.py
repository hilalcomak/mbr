...
def main(testset: str, language_pair: str, seed_no: int, num_samples: int, epsilon_cutoff: float, model: str, subsample_probabilities_model: str, num_subsamples:int,
    ...
    out_path = samples_dir / f"samples.{dataset}.n{num_samples}.epsilon{epsilon_cutoff}.seed{seed_no}.model{model.name}.jsonl"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--probabilities-model', default=None, choices={'facebook/m2m100_1.2B'}, help=' model to get the probabilities from, after subsampling. Ignored if sub-sampling = 1.')
    parser.add_argument('--num-subsamples', default=1, type=int)
